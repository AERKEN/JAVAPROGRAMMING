...
public class WarmUpTasks {
    public static void main(String[] args) {
       ...
       int size= list.size();     //check the old list size

       list.removeAll(Arrays.asList(0));   //remove all 0
        System.out.println(list);

        int newSize= list.size();         //check the size after removed 0

        int totalNumOfZero=size-newSize;       //check how many zero we moved
        System.out.println(totalNumOfZero);

        for (int i = 0; i <totalNumOfZero ; i++) {    //use total number of zero to add zeros back to list
            list.add(0);
